...
describe('Deprecation warning for callback usage (Issue #476)', () => {
  ...
  it('should NOT emit a warning for sync parseICS (no callback)', () => {
    ...
    const deprecationWarnings = warnings.filter(w => w.code === 'NODE-ICAL-CALLBACK');
    assert.equal(deprecationWarnings.length, 0, 'Should not emit deprecation for sync usage');
  });
});
