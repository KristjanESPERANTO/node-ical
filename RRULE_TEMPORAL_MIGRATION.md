...
**Status:** ✅ PR-reif (Stand 9. Dezember 2025)  
**Branch:** `temporal`  
**Tests:** Lokale Suite läuft grün (npm test, 9. Dezember 2025)

> Alle Blocking-Punkte sind erledigt; der Branch ist PR-fertig.
...
1. **RRULE-Parsing-Bug behoben** (Regression eliminiert; Test `parses RRULE with inline DTSTART and preserves BYDAY/COUNT` hinzugefügt). `options.byweekday` liefert `['MO','FR']` (rrule-temporal)
2. **Temporal-Polyfill-Ladereihenfolge korrigiert** (Polyfill vor `rrule-temporal`; `globalThis.Temporal` gesetzt)
3. **TypeScript-Typen an Wrapper angepasst** (`node-ical.d.ts` exportiert Date-basiertes `RRule`-Interface)
4. **Tests dokumentiert** (`npm test` grün, Stand 9. Dezember 2025; Branch `temporal`)
5. **Snapshots/Neue Tests abgeschlossen** (keine Snapshot-Änderungen nötig; Adelaide-DST-Test bleibt; neuer RRULE+DTSTART-Regressionstest aktiv)

Übergreifende Ergebnisse:
- Vollständige Migration zu `rrule-temporal`
- Rückwärtskompatibilität der API
- Alte `rrule` dependency entfernt
- Temporal-basierte RRULE-Expansion mit besserem TZ/DST-Handling
- DST-Bugfix für wiederkehrende Events ([#100](https://github.com/jens-maus/node-ical/issues/100))
