...
function getEventDurationMs(eventData, isFullDay) {
  ...
  return eventData?.start && eventData.end ? new Date(eventData.end).getTime() - new Date(eventData.start).getTime() : 0;
}
...
function collectOverrideInstances(event, {
  ...
  for (const overrideEvent of overrideEvents) {
    ...
    if (!recurrenceKey || (recurrenceKey && seenKeys.has(recurrenceKey))) {
      continue;
    }
