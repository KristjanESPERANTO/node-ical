...
function createRdateParameterFactory({dateParameter}) {
  return function (name) {
    return function (value, parameters, curr) {
      curr[name] ||= [];
      const dates = splitUnescapedCommas(value);

      for (const entry of dates) {
        const temporaryContainer = {};
        dateParameter(name)(entry.trim(), parameters, temporaryContainer);

        const dateValue = temporaryContainer[name];
        if (dateValue instanceof Date) {
          curr[name].push(dateValue);
        }
      }
