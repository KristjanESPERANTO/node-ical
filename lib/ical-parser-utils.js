...
function parseValue(value) {
  ...
  return Number.isNaN(number) ? value.replace(/^"(.*)"$/v, '$1') : number;
}
...
function applyDurationToDate(start, durationString, utcAdd) {
  ...
  while (rest.length > 0) {
    ...
    if (!isSeenTimeDesignator && TIME_ONLY_UNITS.has(unitLetter)) {
      return undefined;
    }
...
function finalizeEndedComponent(component, current, parentStack, {
  ...
  return handleUidEntryInParent(parentEntry, current, storeRecurrenceOverride) ? parentEntry : handleNonUidEntryInParent(parentEntry, current, component, randomIdFactory);
}
...
function normalizeRruleUntil(rruleOnly, startDate, tzUtil) {
  ...
  if (startDate.dateOnly) {
    ...
    return timePart ? rruleOnly.slice(0, untilStart) + `UNTIL=${datePart}` + rruleOnly.slice(untilEnd) : rruleOnly;
  }
...
function addTZFactory(attachTz) {
  return function (dt, parameters) {
    ...
    return dt.tz ? attachTz(dt, dt.tz) : dt;
  };
}
      ...
      if (comps !== null) {
        ...
        if (comps[7] === 'Z') {
          ...
          const fallbackWithStackTimezone = () => {
            ...
            return tzInfo.iana ? tzUtil.parseDateTimeInZone(value, tzInfo.iana) : new Date(year, monthIndex, day, hour, minute, second);
          };
          ...
          if (parameters) {
            ...
            const findTZIDIndex = () => Array.isArray(parameters) ? parameters.findIndex(parameter => typeof parameter === 'string' && parameter.toUpperCase().startsWith('TZID=')) : -1;
