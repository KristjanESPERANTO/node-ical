import windowsZones from '../windowsZones.json' with {type: 'json'};
...
function offsetLabelToMinutes(offset) {
  ...
  if (Number.isNaN(hours) || Number.isNaN(minutes) || minutes >= 60 || hours > 14 || (hours === 14 && minutes !== 0)) {
    return undefined;
  }
...
function resolveZone(zone) {
  return zone ? aliasMap.get(zone) || zone : zone;
}
...
function resolveVTimezoneToIana(vTimezone, year) {
  ...
  // No DST component or equal offsets → fixed-offset zone; try Etc/GMT mapping or return raw offset
  if (stdMins === dstMins || !Number.isFinite(dstMins)) {
  ...
  for (const zone of getZoneNames()) {
    try {
      ...
      if (janOffset !== julOffset && offsets.has(janOffset) && offsets.has(julOffset)) {
