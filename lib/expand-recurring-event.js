...
function isDateInSearchRange(date, searchFrom, searchTo, isFullDay) {
  if (isFullDay) {
    const dateKey = generateDateKey(date, true);
    return dateKey >= generateDateKey(searchFrom, true)
      && dateKey <= generateDateKey(searchTo, true);
  }

  return date >= searchFrom && date <= searchTo;
}

/**
 * Collect RRULE and RDATE occurrence starts within the search range.
 * @param {object} event
 * @param {Date} searchFrom
 * @param {Date} searchTo
 * @param {boolean} isFullDay
 * @returns {Date[]} Unique occurrence starts in the search range
 */
function collectRecurringDates(event, searchFrom, searchTo, isFullDay) {
  const dates = event.rrule
    ? event.rrule.between(searchFrom, searchTo, true)
    : [];

  if (Array.isArray(event.rdate)) {
    for (const rdate of event.rdate) {
      const date = rdate instanceof Date ? rdate : new Date(rdate);
      if (!Number.isNaN(date.getTime())
        && isDateInSearchRange(date, searchFrom, searchTo, isFullDay)) {
        dates.push(date);
      }
    }
  }

  if (!event.rrule && Array.isArray(event.rdate) && event.start instanceof Date
    && isDateInSearchRange(event.start, searchFrom, searchTo, isFullDay)) {
  ...
  return dates
    .toSorted((a, b) => a - b)
    .filter(date => {
      const key = isFullDay ? generateDateKey(date, true) : date.toISOString();
      if (seen.has(key)) {
        return false;
      }
...
function expandRecurringEvent(event, options) {
  ...
  if (!event.rrule && !Array.isArray(event.rdate)) {
  ...
  const dates = collectRecurringDates(event, searchFrom, searchTo, isFullDay);
  const instances = [];
