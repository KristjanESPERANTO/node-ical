...
function mergeLegacyOverrides(zoneTable, oldMap) {
  ...
  for (const key of Object.keys(oldMap)) {
    let entry = zoneTable[key];
    entry ||= {iana: []};

    const iana = getFirstIanaFromLookup(zoneTable, oldMap[key]);
    // Preserve legacy behavior: push even when undefined so JSON.stringify yields [null]
    if (entry.iana.length === 0) {
      entry.iana.push(iana);
    }
...
try {
  ...
  let zoneTable = buildZoneTable(zones);
  const oldMap = readOldMap(OLD_MAP_PATH);
  zoneTable = mergeLegacyOverrides(zoneTable, oldMap);
  writeOutput(OUTPUT_PATH, zoneTable);
